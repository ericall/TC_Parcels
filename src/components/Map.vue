<template>
  <div id="viewDiv" 
    ...
    v-bind:class="{ minMap: minimizeMap }"></div>
</template>
...
export default {
  ...
  methods: {
    ...
    executeIdentifyTask(event) {
      ...
      let identifyTask = new this.esriMods.IdentifyTask(
        config.MetroGISUrl
      );
    ...
    loadModules(
      ...
      ([
        ...
        var mapLayer = new MapImageLayer({
          url:
            //  "http://gis.hennepin.us/arcgis/rest/services/Maps/ADOPT_A_HIGHWAY/MapServer",
            config.MetroGISUrl,
          sublayers: [
            {
              id: 1,
